...
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`python`）。当前版本 `1.0.5`。

## 1. 这是什么

Python 语言支持。**纯声明式插件**：给编辑器挂 pyright LSP，自己没有任何界面。

**用户在哪看到它**：不出现在任何区域（没有 `views`、没有 `appearsIn`）。它的「界面」是编辑器里的高亮、跳转、补全。
